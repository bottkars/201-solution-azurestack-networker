...
$account_available = Get-AZStorageAccountNameAvailability -Name $ImageStorageAccount -ErrorAction SilentlyContinue 
...
if ($account_free -eq $true) {
    try {
        ...
        $RG = Get-AZResourceGroup -Name $ImageRG -Location local -ErrorAction Stop  
    }
    catch {
        Write-Host -ForegroundColor yellow [need to create]
        Write-Host -ForegroundColor White -NoNewline "Creating Image RG $ImageRG"        
        $RG = New-AZResourceGroup -Name $ImageRG -Location $location
        Write-Host -ForegroundColor Green [Done]
    }
    $newAcsaccount = New-AZStorageAccount -ResourceGroupName $ImageRG `
        -Name $ImageStorageAccount -Location $location `
        -Type $storageType # -ErrorAction SilentlyContinue
    if (!$newAcsaccount) {
        $newAcsaccount = Get-AZStorageAccount -ResourceGroupName $ImageRG | Where-Object StorageAccountName -match $ImageStorageAccount
    }    
    $newAcsaccount | Set-AZCurrentStorageAccount
...
try {
    Add-AZVhd -ResourceGroupName $ImageRG -Destination $urlOfUploadedImageVhd `
        -LocalFilePath $Image -OverWrite:$false  -NumberOfUploaderThreads 32 #-ErrorAction SilentlyContinue 
}
catch {
...
if ($TESTONLY.IsPresent) {
    Test-AZResourceGroupDeployment -ResourceGroupName $resourceGroup -Mode Incremental -TemplateFile $PSScriptRoot/azuredeploy.json -TemplateParameterObject $parameters
}
else {
    New-AZResourceGroupDeployment -Name $resourceGroup -ResourceGroupName $resourceGroup -Mode Incremental -TemplateFile $PSScriptRoot/azuredeploy.json -TemplateParameterObject $parameters
}